...
import type { Organization } from '../../types/control-plane.js';
 ...
 */
export function getTenantClient(org: Pick<Organization, 'id' | 'supabase_url' | 'supabase_service_key'>): SupabaseClient {
  if (!org.supabase_url || !org.supabase_service_key) {
    throw new Error(`Organization ${org.id} is not fully provisioned (missing Supabase credentials)`);
  }
