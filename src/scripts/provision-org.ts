 ...
 *
 * Usage: npx tsx src/scripts/provision-org.ts --name "Museum of Art" --slug museum-of-art --plan-tier basic
 */
...
async function createSupabaseProject(name: string, dbPassword: string): Promise<SupabaseProject> {
...
async function waitForProject(projectRef: string, maxWaitMs = 120_000): Promise<void> {
  ...
  while (Date.now() - start < maxWaitMs) {
    ...
    console.log(`  Waiting for project to be ready (status: ${project.status})...`);
    await new Promise(resolve => setTimeout(resolve, 5000));
  }
  throw new Error(`Project ${projectRef} did not become ready within ${maxWaitMs / 1000}s`);
}

async function getApiKeys(projectRef: string): Promise<{ anon: string; service_role: string }> {
...
export async function provisionOrg(options: ProvisionOptions): Promise<string> {
  ...
  try {
    ...
    if (existing.length > 0) {
      throw new Error(`Organization with slug '${options.slug}' already exists`);
    }

    // Create org record in provisioning state
    const { rows: [org] } = await cpClient.query(
...
async function main() {
  ...
  if (!parsed.name || !parsed.slug) {
    console.error('Usage: provision-org.ts --name "Org Name" --slug org-slug [--plan-tier basic]');
    process.exit(1);
  }
