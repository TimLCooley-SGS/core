 ...
 *
 * Usage:
 *   npx tsx src/scripts/manage-org.ts suspend --slug museum-of-art
 *   npx tsx src/scripts/manage-org.ts activate --slug museum-of-art
 *   npx tsx src/scripts/manage-org.ts archive --slug museum-of-art
 *   npx tsx src/scripts/manage-org.ts delete --slug museum-of-art   (dev only)
 */
...
async function manageOrg(action: Action, slug: string) {
  ...
  if (!cpDatabaseUrl) throw new Error('CONTROL_PLANE_DATABASE_URL is required');
  ...
  try {
    ...
    const org = rows[0] as { id: string; name: string; status: string; supabase_project_id: string | null };

    switch (action) {
      case 'suspend': {
        if (org.status !== 'active') {
          console.error(`Cannot suspend org in '${org.status}' state. Must be 'active'.`);
          process.exit(1);
        }
        ...
        if (org.supabase_project_id) {
          ...
          await supabaseFetch(`/projects/${org.supabase_project_id}/pause`, { method: 'POST' });
        }
      ...
      case 'activate': {
        if (org.status !== 'suspended') {
          console.error(`Cannot activate org in '${org.status}' state. Must be 'suspended'.`);
          process.exit(1);
        }
        ...
        if (org.supabase_project_id) {
          ...
          await supabaseFetch(`/projects/${org.supabase_project_id}/restore`, { method: 'POST' });
        }
      ...
      case 'delete': {
        if (process.env.NODE_ENV === 'production') {
          console.error('Cannot delete orgs in production. Use archive instead.');
          process.exit(1);
        }
        ...
        if (org.supabase_project_id) {
          ...
          const res = await supabaseFetch(`/projects/${org.supabase_project_id}`, { method: 'DELETE' });
          if (!res.ok) {
            console.warn(`Warning: Failed to delete Supabase project (${res.status}). Continuing cleanup...`);
          }
        }

        // Clean up control plane records
        await cpClient.query('DELETE FROM identity_org_links WHERE organization_id = $1', [org.id]);
        await cpClient.query('DELETE FROM impersonation_sessions WHERE organization_id = $1', [org.id]);
        await cpClient.query('DELETE FROM organizations WHERE id = $1', [org.id]);

        await cpClient.query(
          `INSERT INTO platform_audit_log (action, resource_type, resource_id, metadata)
           VALUES ('org.deleted', 'organization', $1, $2)`,
          [org.id, JSON.stringify({ name: org.name, slug, env: 'development' })],
        );
...
async function main() {
  ...
  if (!validActions.includes(action)) {
    console.error(`Usage: manage-org.ts <${validActions.join('|')}> --slug <slug>`);
    process.exit(1);
  }
