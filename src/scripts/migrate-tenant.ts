 ...
 * Usage: npx tsx src/scripts/migrate-tenant.ts <database-url>
 *   or:  npx tsx src/scripts/migrate-tenant.ts --org-slug <slug>
 *
...
export async function runTenantMigrations(databaseUrl: string, options?: { seed?: boolean }) {
  ...
  try {
    ...
    if (options?.seed) {
      ...
      for (const file of seedFiles) {
        ...
        try {
          await client.query(sql);
          await client.query('INSERT INTO _migrations (name) VALUES ($1)', [`seed:${file}`]);
          await client.query('COMMIT');
        } catch (err) {
          await client.query('ROLLBACK');
...
async function main() {
  ...
  if (args[0] === '--org-slug') {
    ...
    if (!cpUrl) {
      console.error('CONTROL_PLANE_DATABASE_URL is required for --org-slug mode');
      process.exit(1);
    }
