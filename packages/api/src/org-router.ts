...
import { getTenantClient, getTenantClientForUser } from "./tenant";
import type { Organization, IdentityOrgLink } from "@sgscore/types";
 ...
 */
export async function getLinksForIdentity(
  globalIdentityId: string,
): Promise<(IdentityOrgLink & { organization: Organization })[]> {
  const cp = getControlPlaneClient();
  const { data, error } = await cp
  ...
  if (error)
    throw new Error(`Failed to fetch org links: ${error.message}`);
...
export async function getTenantClientBySlug(
  slug: string,
): Promise<{ client: SupabaseClient; org: Organization } | null> {
