...
import type { Capability, StaffCapabilityOverride } from "@sgscore/types";
...
export async function resolveCapabilities(
  ...
  if (assignError)
    throw new Error(
      `Failed to fetch staff assignments: ${assignError.message}`,
    );
  ...
  if (roleCapResult.error)
    throw new Error(
      `Failed to fetch role capabilities: ${roleCapResult.error.message}`,
    );
  if (overridesResult.error)
    throw new Error(
      `Failed to fetch overrides: ${overridesResult.error.message}`,
    );
  ...
  // Apply overrides
  const overrides = (overridesResult.data ?? []) as Pick<
    StaffCapabilityOverride,
    "capability_id" | "override_type"
  >[];
