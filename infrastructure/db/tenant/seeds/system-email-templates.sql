-- Seed 8 system email templates (idempotent via ON CONFLICT)
...
VALUES
  ...
  (
    'Donation: Thank You',
    'Thank you for your donation to {{org_name}}!',
    true,
    'donation_thank_you',
    'active',
    '[
      {"id":"sys-hdr-8","type":"header","props":{"logoUrl":"","logoAlt":"","logoWidth":150,"title":"{{org_name}}","backgroundColor":"#4E2C70","textColor":"#ffffff","alignment":"center"}},
      {"id":"sys-txt-8","type":"text","props":{"html":"<p>Hi {{first_name}},</p><p>Thank you so much for your generous donation of <strong>{{donation_amount}}</strong> to {{org_name}}!</p><p><strong>Date:</strong> {{donation_date}}<br/><strong>Campaign:</strong> {{donation_campaign}}<br/><strong>Receipt #:</strong> {{receipt_number}}</p><p>Your support makes a real difference. We truly appreciate your generosity.</p>","color":"#374151","alignment":"left"}},
      {"id":"sys-ftr-8","type":"footer","props":{"html":"","companyName":"{{org_name}}","address":"","color":"#6b7280","backgroundColor":"#f9fafb","alignment":"center"}}
    ]'::jsonb,
    '{"backgroundColor":"#f4f4f5","contentWidth":600,"fontFamily":"Arial, Helvetica, sans-serif"}'::jsonb
  )
